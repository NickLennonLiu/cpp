...
public:
    ...
    int remove(int cursor)
    {
        ...
        ListNode* temp = cursors[0][cursor]->nex;  // 保存光标下一个位置
    ...
    void show()
    {
        ListNode *iter = forward.first();     // 从正向表的首节点开始遍历
        while (iter != forward.trailer &&
                iter != backward.trailer &&
                iter != forward.header &&   // 直到遇到某个哨兵节点
                iter != backward.header 
                )
        {
            putchar(iter->val);
            iter = iter->nex;
