...
Node* findRank(int r, int& cur_rank)
{
	...
	while (count + p->size() < r + 1 && p != END)	// END->size() = 1
	{
		count += p->size();
		p = p->next;
	}
...
int main()
{
	...
	for (int i = 0; i < strlen(a); i += blockSize)
	{
		...
		last->next = tmp;
		last = tmp;
		cout << "debug: blocksize: " << tmp->size() << endl;
	}
	last->next = END;
	END->prev = last;
	...
	while (m--)
	{
		...
		who = findRank(index, cur_rank);				//从哪个节点开始 应该没问题
		...
		{
			int count1 = 0;	
			Node* start = lParent, * end = rParent;	// 相消发生时的左节点和右节点
			cout <<"debug: start: " << start->id << " end: " << end->id << endl;
			...
			while (left == right)
			{
				...
				if (start->size() != 0)
				{
					cout << "debug: start id: " << start->id << endl;
